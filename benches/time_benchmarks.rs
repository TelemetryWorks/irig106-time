...
use irig106_time::secondary::{validate_secondary_checksum, parse_secondary_header, SecHdrTimeFormat};
use irig106_time::intra_packet::{parse_intra_packet_time, IntraPacketTimeFormat};

const WARMUP: u64 = 1_000;
const ITERS: u64 = 100_000;

struct Bench { name: &'static str, total: Duration, iters: u64 }

impl Bench {
    fn ns(&self) -> f64 { self.total.as_nanos() as f64 / self.iters as f64 }
}
...
fn bench<F: FnMut()>(name: &'static str, mut f: F) -> Bench {
    ...
    for _ in 0..ITERS { f(); }
    Bench { name, total: start.elapsed(), iters: ITERS }
}
...
fn sec_hdr_buf() -> [u8; 12] {
    ...
    let mut sum: u32 = 0;
    for i in 0..5 { sum = sum.wrapping_add(u16::from_le_bytes([b[i*2], b[i*2+1]]) as u32); }
...
fn build_correlator(n: usize) -> TimeCorrelator {
    ...
    for i in 0..n {
        c.add_reference(1, Rtc::from_raw((i as u64) * 10_000_000),
...
fn main() {
    ...
    r.push(bench("rtc_from_le_bytes", || { black_box(Rtc::from_le_bytes(black_box(bytes))); }));
    r.push(bench("rtc_from_raw", || { black_box(Rtc::from_raw(black_box(0xABCDEF123456u64))); }));
    let rtc = Rtc::from_raw(12345678);
    r.push(bench("rtc_to_nanos", || { black_box(black_box(rtc).to_nanos()); }));
    let (a, b2) = (Rtc::from_raw(1_000_000), Rtc::from_raw(2_000_000));
    r.push(bench("rtc_elapsed_ticks", || { black_box(black_box(a).elapsed_ticks(black_box(b2))); }));
    r.push(bench("rtc_elapsed_nanos", || { black_box(black_box(a).elapsed_nanos(black_box(b2))); }));
    ...
    r.push(bench("csdw_from_le_bytes", || { black_box(TimeF1Csdw::from_le_bytes(black_box(cb))); }));
    let csdw = TimeF1Csdw::from_raw(0x0351);
    r.push(bench("csdw_all_fields", || {
        let c = black_box(csdw);
        black_box(c.time_source()); black_box(c.time_format());
        black_box(c.is_leap_year()); black_box(c.date_format());
    }));
    ...
    r.push(Bench { name: "corr_detect_jumps_3600", total: start.elapsed(), iters: 1_000 });

    // AbsoluteTime
    let t = AbsoluteTime::new(100, 12, 30, 25, 340_000_000).unwrap();
    r.push(bench("abs_add_nanos", || { black_box(black_box(t).add_nanos(black_box(15_000_000))); }));
    r.push(bench("abs_sub_nanos", || { black_box(black_box(t).sub_nanos(black_box(15_000_000))); }));
    r.push(bench("abs_total_nanos_of_day", || { black_box(black_box(t).total_nanos_of_day()); }));
    ...
    for b in &r {
        ...
        let os = if ops > 1e6 { format!("{:.1}M", ops/1e6) }
                 else if ops > 1e3 { format!("{:.1}K", ops/1e3) }
                 else { format!("{:.0}", ops) };
